...
void Application::setup(){
	...
	mTransformID[0] = glGetUniformLocation(oPlane.shader[0], "mTransform");
	fTimeID[0] = glGetUniformLocation(oPlane.shader[0], "fTime");
	eyeID[0] = glGetUniformLocation(oPlane.shader[0], "vEye");
	myLightPositionID[0] = glGetUniformLocation(oPlane.shader[0], "myLightPosition");
	cameraID[0] = glGetUniformLocation(oPlane.shader[0], "camera");
	perspectiveID[0] = glGetUniformLocation(oPlane.shader[0], "perspective");



	//GourudShading
	strVertexShader = loadTextFile("Shaders/gouradPlane.v");
	strFragmentShader = loadTextFile("Shaders/passThru.f");
	InitializeProgram(oPlane.shader[1], strVertexShader, strFragmentShader);

	mTransformID[1] = glGetUniformLocation(oPlane.shader[1], "mTransform");
	fTimeID[1] = glGetUniformLocation(oPlane.shader[1], "fTime");
	eyeID[1] = glGetUniformLocation(oPlane.shader[1], "vEye");
	myLightPositionID[1] = glGetUniformLocation(oPlane.shader[1], "myLightPosition");
	cameraID[1] = glGetUniformLocation(oPlane.shader[1], "camera");
	perspectiveID[1] = glGetUniformLocation(oPlane.shader[1], "perspective");
...
void Application::display() {
	...
	glUseProgram(oPlane.shader[currentShader]);
	...
	glUniformMatrix4fv(mTransformID[currentShader], 1, false, glm::value_ptr(transform));
	glUniformMatrix4fv(cameraID[currentShader], 1, false, glm::value_ptr(lookAt));
	glUniformMatrix4fv(perspectiveID[currentShader], 1, false, glm::value_ptr(perspective));



	//parametro de fase para shaders
	glUniform1f(fTimeID[currentShader], glm::radians(time));
	glUniform3fv(eyeID[currentShader], 1, glm::value_ptr(eye));
	newLight = myLightPosition;
	//newLight.z = myLightPosition.z * cos(glm::radians(time));
	glUniform3fv(myLightPositionID[currentShader], 1, glm::value_ptr(newLight));


	//glUniform3fv(oPlane.uEye, 1, glm::value_ptr(eye));
	glDrawArrays(GL_TRIANGLES, 0, oPlane.getNumVertex());

	glUseProgram(cube.shaders);
...
void Application::keyboard(int key, int scancode, int action, int mods){
	if (action == GLFW_PRESS)
	{
		...
		case GLFW_KEY_LEFT:
			...
			break;
		case GLFW_KEY_P:
			currentShader = (currentShader + 1) % oPlane.numShaders;
			std::cout << "Shader: " << currentShader;
			break;
		}
	}
}
...
void Application::crearCubo()
{
	...
	InitializeProgram(cube.shaders, strVertexShader, strFragmentShader);
	idTransform = glGetUniformLocation(cube.shaders, "transform");
